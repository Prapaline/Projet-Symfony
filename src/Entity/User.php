...
class User implements UserInterface, PasswordAuthenticatedUserInterface
{
    ...
    public function addDateDebut(Reservation $dateDebut): static
    {
        if (!$this->dateDebut->contains($dateDebut)) {
            $this->dateDebut->add($dateDebut);
            $dateDebut->setUsers($this);
        }

        return $this;
    }

    public function removeDateDebut(Reservation $dateDebut): static
    {
        if ($this->dateDebut->removeElement($dateDebut)) {
            // set the owning side to null (unless already changed)
            if ($dateDebut->getUsers() === $this) {
                $dateDebut->setUsers(null);
            }
        }
    ...
    public function addFavori(Vehicule $vehicule): static
    {
        if (!$this->favoris->contains($vehicule)) {
            $this->favoris[] = $vehicule;
        }

        return $this;
    }

    public function removeFavori(Vehicule $vehicule): static
    {
        $this->favoris->removeElement($vehicule);
